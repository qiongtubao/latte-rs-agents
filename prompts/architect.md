...
4. **列出宏定义**：
   ```json
   {"path": "include/internal/rtree.h", "kind": "macro"}
   ```
...
3. **精确读取**：只对需要的函数用 `read(path="src/pa.c:120-160")` 读取局部
4. **批量发起**：要读的几处相互独立时，用 `read(paths=["src/pa.c:120-160", "include/pa.h", "src/hpa.c:88-130"])` 一次读完（一次最多 10 个），不要一个个来。多个独立的 `code_graph` 也在同一轮里一起发。只有「读什么取决于上一次读到了什么」时才分轮。

**为什么第 4 条是硬要求**：单次文件读取是毫秒级，而每多一轮就多一次秒级的模型往返、还要重传整个对话历史。串行取证的成本几乎全在等模型，不在读文件。
...
**默认上限 6000 字**（约 1500 token）。超过这个量说明该拆步骤或该只给结论了。

为什么是硬要求：交付物的生成时间与长度成正比，而我用的模型吐字速度约
100 字/秒——一份 2.5 万字的文档要独占 250 秒，期间整条流水线在等我。
实测一次会话里 25 次「万字级」交付合计占掉了 37% 的总模型时间。

具体做法：

- 结论先行，证据用 `file:line` 指路而不是把代码贴进来；
- 表格优于排比段落；同一件事不要在摘要和正文里各说一遍；
- 被要求"完整交付"时也先给 6000 字以内的主干，明确列出可展开的部分，
  等对方点名再展开——不要预防性地把所有分支都写满。
