...
import type { StageStore } from "./types";
...
/**
 * 一条已解析的重放记录：捕获的事件 + 其持久化的接收时间戳（unix 毫秒）。
 * 当前落盘格式不给每个事件单独写时间戳，故 timestampMs 会回退到最近一条
 * meta 的 created_at_unix_ms（见 parseJsonlRecords）。
 */
...
/**
 * 用于把重放结果播种进 store 的回调。传入的是完整的 ReplayRecord 列表
 * （事件 + 持久化时间戳），使 store 能在订阅实时流之前完整重建 Stage 树
 * 且保留正确的时间戳。与 stageStore 的 `seed(...)` action 对接。
 */
export type StageSeeder = (records: ReplayRecord[]) => void;
 ...
 */
function timestampFrom(raw: Record<string, unknown>, fallback: number): number {
  return (
    numberField(raw.timestampMs) ??
 ...
 * 无法识别时返回 null（调用方会跳过）。
 */
function eventFrom(raw: Record<string, unknown>): ChatEvent | null {
  if (isObject(raw.event) && typeof raw.event.type === "string") {
    return raw.event as unknown as ChatEvent;
  }
 ...
 */
export function parseJsonlRecords(jsonl: string): ReplayRecord[] {
  ...
  for (const line of jsonl.split(/\r?\n/)) {
    ...
    if (raw.type === META_TYPE) {
      ...
      if (created !== undefined) lastTs = created;
      continue;
    }

    const event = eventFrom(raw);
    if (!event) continue; // 无法识别的记录：跳过而不抛错
 ...
 * 无需每条重建 ctx。
 */
export function foldRecords(records: ReplayRecord[]): StageStore {
 ...
 */
export function replayJsonl(jsonl: string): StageStore {
 ...
 */
export function seedStoreFromRecords(
 ...
 */
export function seedStoreFromJsonl(
  jsonl: string,
  seed: StageSeeder,
): StageStore {
...
export function recordsToEvents(records: ReplayRecord[]): ChatEvent[] {
